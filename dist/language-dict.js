 ...
 * learning + seed only.
 */
export function getDictionary(iso) {
    ...
    try {
        ...
        const spell = nspell(readFileSync(join(dir, "index.aff"), "utf8"), readFileSync(join(dir, "index.dic"), "utf8"));
        const check = (word) => spell.correct(word);
        cache.set(key, check);
        return check;
    }
    catch {
