...
export function extractLinks(html, baseUrl) {
    ...
    $("a[href]").each((_i, el) => {
        ...
        if (!u || !u.startsWith(origin) || u === baseUrl)
            return; // off-site or self links
        links.add(u);
    });
    return [...links];
}
 ...
 */
export async function crawlSite(startUrls, opts) {
    const browser = await chromium.launch();
    const context = await browser.newContext(opts.authPath ? { storageState: opts.authPath } : {});
