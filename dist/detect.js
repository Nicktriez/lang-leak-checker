 ...
 */
function getDetector(target, other) {
    ...
    try {
        ...
        throw new Error(`language '${target}' is not compiled into the wasm detector — rebuild it with: lingua-wasm/build.sh (and add '${target}' to src/languages.ts if needed)`);
    }
}
/** Best (highest-confidence) language for `text`, or undefined if none. */
function bestOf(detector, text) {
    const best = detector.computeLanguageConfidenceValues(text)[0];
...
export function learnAdoptedWords(nodes, targetIso, minLength) {
    ...
    for (const node of nodes) {
        ...
        if (!best || best.value < CONFIDENCE_THRESHOLD || best.language === target.lingua)
            continue;
 ...
 */
export function detectLeaks(nodes, targetIso, opts, allowlist) {
    const { target, other } = resolveLanguages(targetIso);
    const detector = getDetector(target.lingua, other.lingua);
    ...
    const allowed = (part) => {
        if (!/^\p{L}+$/u.test(part))
            return false; // skip whitespace/punct
