...
function bestOf(text) {
    return detector.computeLanguageConfidenceValues(text)[0];
}
/** Maps a CLI ISO-639-1 code to the lingua name used internally. */
export function targetLanguageName(target) {
 ...
 */
export function learnAdoptedWords(nodes, target, // lingua name, e.g. "Danish"
minLength) {
    ...
    for (const node of nodes) {
        ...
        if (!best || best.value < CONFIDENCE_THRESHOLD || best.language === target)
            continue;
        const tokens = alphaTokens(node.text);
        if (tokens.length < 2)
            continue; // lone words can't teach us ("Leaderboard" vs "Upload")
 ...
 */
export function detectLeaks(nodes, target, opts, allowlist) {
